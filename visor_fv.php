...
<div id="fb-root"></div>
<script async defer crossorigin="anonymous" src="https://connect.facebook.net/es_ES/sdk.js#xfbml=1&version=v20.0" nonce="7awkm4KP"></script>
<?php
    $id = $_GET['id'];
        include 'conexion.php';
...
<script>
var panoramaViewer = pannellum.viewer('panorama', {
    ...
    while($obj=pg_fetch_object($consulta)){
        ...
        $long_p = $obj->longitud;
        $nombre_p = $obj->nombre;   
        $descripcion = substr($obj->descrip, 0, 150);
        echo "\"panorama\": \"galeria/panorama-".$obj->id_sitios.".jpg\",";
    }
    ?>
    "autoLoad": true,
    "autoRotate": -2,
    "hotSpots": [
        ...
        // Primer conjunto de hotspots
        <?php
            $query="SELECT * FROM hotspot WHERE id_origen = $id limit 88";
            $consulta=pg_query($conexion,$query);
            while($obj=pg_fetch_object($consulta)){
                echo "
            ...
            'clickHandlerFunc': function() {
                window.location.href = 'clasifier.php?id=".$obj->id_destino."&lat=".$obj->latitud."&long=".$obj->longitud."';
                }
            },
        ...
        // Segundo conjunto de hotspots (hotspot2)
         <?php
            $query = "SELECT * FROM hotspot WHERE id_origen = $id and type = 'card' limit 1";
            $consulta = pg_query($conexion, $query);
            while ($obj = pg_fetch_object($consulta)) {
                echo "
                {
...
// Función para crear el hotspot con una imagen personalizada
function hotspotIcon(hotSpotDiv, args) {
    var img = document.createElement('img');
    img.src = args.img; // Aquí se establece la imagen del hotspot
    img.style.width = '32px'; // Puedes ajustar el tamaño de la imagen
...
function openModal(hotspotId) {
        ...
        // Use AJAX to fetch the hotspot information
        var xhr = new XMLHttpRequest();
        xhr.open('GET', 'get_hotspot_info_i.php?id=' + hotspotId, true);
        xhr.onload = function() {
            if (xhr.status === 200) {
                modalContent.innerHTML = xhr.responseText;
                modal.style.display = "block";
            } else {
                modalContent.innerHTML = '<p>Error loading hotspot information.</p>';
            }
        };
        xhr.send();
    }
...
?>
<div class=leyenda_filtro_v><img src=imagenes/sonidos.png height=15px> Foto 360 | <a href="routing.php?id=<?php echo $id;?>&lat_i=<?php echo $latitud;?>&long_i=<?php echo $longitud;?>&lat_f=<?php echo $latitud_f;?>&long_f=<?php echo $longitud_f;?>">¿Cómo llegar?</a>
<br>
</div>
...
<hr>
    ...
    <?php 
        ...
        while($obj=pg_fetch_object($consulta)){
            ...
            $consulta=pg_query($conexion,$query);
            while($obj=pg_fetch_object($consulta)){
                $contador_id = $obj->contadorx;
		}
            ;?>
            <div class="centrado"><button onclick="window.modal1.close();">Cerrar</button></div>
            </dialog>
            ...
            <form action="comentarios.php" method="post">
                ...
                <input type="hidden" name="id" value="<?php echo $contador_id;?>">
                <br><div class="centrado"><input class="enlace_e" type="submit" value="Enviar"> <button onclick="window.modal2.close();">Cerrar</button></div>
...
<div class="learn"><img src="imagenes/learn.png" alt="Controles 360"></div>
<div class="video_opc"><a href="visor_v.php?id=<?php echo $id;?>" title="Contenido adicional"><img src="imagenes/video.png" width="150px" alt="Video adicional"></a></div>
